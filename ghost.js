...
class Ghost {
    constructor(startTile, speed, board, pacman, ghostActor, findPath) {
        ...
        this.path = [];
        this.isMoving = false;
        this.spriteSheet = SPRITESHEET.actors.filter(item => item.actor == ghostActor)[0];
    ...
    newPath() {
        ...
        this.path = []
        if (this.findPath(getTile(this.pixel), getTile(this.pacman.pixel), [], this.path, 50)) {
    ...
    draw() {
        ...
        ctx.drawImage(
            ...
            this.pixel.x - GHOST_SIZE / 2 + 2, this.pixel.y - GHOST_SIZE / 2, GHOST_SIZE, GHOST_SIZE
        );
    }
}
